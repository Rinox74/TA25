...
export const localStorageService = {
  ...
  addEvent: async (eventData: Omit<Event, 'id'>): Promise<Event> => {
    ...
    data.notifications.unshift({ id: uuidv4(), type: NotificationType.NEW_EVENT, message: `Nuovo evento: ${newEvent.title}`, relatedId: newEvent.id, is_read: false, createdAt: new Date().toISOString() });
    saveData(data);
    return newEvent;
  },
  ...
  addArticle: async (articleData: Omit<Article, 'id' | 'createdAt'>): Promise<Article> => {
    ...
    data.notifications.unshift({ id: uuidv4(), type: NotificationType.NEW_ARTICLE, message: `Nuovo articolo: ${newArticle.title}`, relatedId: newArticle.id, is_read: false, createdAt: new Date().toISOString() });
    saveData(data);
    return newArticle;
  },
  ...
  markNotificationsAsRead: async (): Promise<void> => {
      const data = getData();
      data.notifications.forEach((n: AppNotification) => n.is_read = true);
      saveData(data);
  },
